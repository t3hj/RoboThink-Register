...
export default function Register() {
  ...
  async function completeLesson(student: RosterEntry) {
    ...
    setBusyId(student.id)
    const { error: rpcErr } = await supabase.rpc('complete_current_lesson', {
      in_student_id: student.id,
      in_date: date,
    })
    if (rpcErr) {
      setBusyId(null)
      notify(rpcErr.message, 'error')
      return
    }
    const ok = await upsertAttendance(student, { status: 'Completed', time_out: nowHM() })
    setBusyId(null)
    if (ok) {
      notify(`Lesson recorded for ${student.full_name}`, 'success')
      void loadRoster(date)
    }
  }
  ...
  return (
    <div>
      ...
      {roster.length === 0 ? (
        ...
        <div className="card divide-y divide-slate-100">
          {roster.map((s) => {
            ...
            const needsAction = prog && prog.current_kind !== 'normal' && prog.current_kind !== 'complete'
            const canCompleteLesson = prog?.current_kind === 'normal' && prog.current_lesson_number != null
              ...
              <div key={s.id} className="p-3 sm:p-4 flex flex-col gap-3">
                  ...
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-slate-500 flex flex-wrap gap-x-2">
                        <span>{prog ? levelLabel({ name: prog.level_name ?? '' }) : s.levels?.name ?? '—'}</span>
                      ...
                      {!att || att.status === 'Not Arrived' ? (
                        ...
                        <>
                          ...
                          <button
                            ...
                            title={needsAction ? 'Resolve the assessment/remediation below first' : undefined}
                ...
                {needsAction && (
                  <AssessmentPanel
                    ...
                    focusTopic={prog?.focus_topic ?? null}
                    plan={s.plan}
      ...
      <ConfirmDialog
        ...
        message={
          confirmTarget
            ? `Record lesson ${confirmTarget.progress?.current_lesson_number} as completed for ${confirmTarget.full_name}, attributed to ${profile?.name ?? 'you'}, and mark them as Completed on the register?`
            : ''
        }
